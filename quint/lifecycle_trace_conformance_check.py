#!/usr/bin/env python3
"""Fail when lifecycle Quint trace replays diverge from documented contracts."""
...
REQUIRED_CASES = {
    ...
    "close-live-stmt",
    "close-v2-live-stmt",
    "prepare-v2-v3-zero-flags",
    "close-live-backup",
    "close-v2-live-backup",
    "backup-step-done-finish-ok",
    "backup-finish-incomplete-ok",
    "backup-step-zero-no-progress",
    "backup-step-negative-all-remaining-done",
    "backup-step-transient-conflict-retry",
    "backup-init-same-connection-error",
    "backup-init-destination-read-transaction-error",
}
...
def check(divergences: list[tuple[str, str, frozenset[str]]]) -> None:
    ...
    raise ValueError("unexpected lifecycle divergences: " + "; ".join(rendered))
...
def main() -> int:
    try:
        ...
        print(f"lifecycle trace conformance check failed: {err}", file=sys.stderr)
        return 1

    print(
        "lifecycle trace conformance check passed: "
