...
from trace_oracles import (
    expected_deserialize_rc as oracle_expected_deserialize_rc,
    expected_steps as oracle_expected_steps,
    staged_terminal_stage as oracle_staged_terminal_stage,
    terminal_facts as oracle_terminal_facts,
)
from trace_scenarios import (
    ...
    infer_lifecycle_transition_steps,
    infer_serde_transition_steps,
    infer_stmt_transition_steps,
    scenario_family,
)
...
def validate_terminal_facts(
    ...
    extra_keys = sorted(set(state) - set(terminal_facts))
    if extra_keys:
        raise ValueError(f"{scenario}: terminal state has unexpected keys {extra_keys!r}")
...
def infer_serde_model(scenario: str, states: list[dict[str, object]]) -> dict[str, object]:
    ...
    observed_deserialize_rc = rcs[0]
    if not isinstance(observed_deserialize_rc, str) or observed_deserialize_rc not in VALID_RC:
        raise ValueError(f"unsupported observed deserialize rc value: {observed_deserialize_rc!r}")

    expected_rc = oracle_expected_deserialize_rc(scenario)
    if observed_deserialize_rc != expected_rc:
        raise ValueError(
            f"{scenario}: expected sqlite3_deserialize rc {expected_rc!r}, "
            f"got {observed_deserialize_rc!r}"
        )

    info = SERDE_SCENARIOS[scenario]
    inferred_steps = infer_serde_transition_steps(states)
    validate_expected_steps(scenario, inferred_steps, oracle_expected_steps(scenario))
    validate_terminal_facts(scenario, states[-1], oracle_terminal_facts(scenario))
...
def infer_lifecycle_model(scenario: str, states: list[dict[str, object]]) -> dict[str, object]:
    ...
    expected_terminal_stage = oracle_staged_terminal_stage(scenario)
    validate_staged_trace(
        scenario,
        states,
        expected_terminal_stage,
        oracle_terminal_facts(scenario),
    )
...
def emit_tcl_serde(model: dict[str, object], out_path: Path) -> None:
    ...
    expected_rc = str(model["expected_rc"])
    expected_ok = "1" if expected_rc == "SQLITE_OK" else "0"
    case_name = str(model["case_name"])

    setup_block = ""
    if str(model["setup"]) == "read_txn":
        setup_block = "db eval {BEGIN; SELECT count(*) FROM t1;}"
    elif str(model["setup"]) == "backup_source":
        setup_block = "sqlite3 bdst :memory:\nset bk [sqlite3_backup B bdst main db main]"
...
# NOTE: Tcl wrappers do not expose every C rc directly. This executable scaffold
# checks success/failure class; the generated C harness is the authoritative
# rc-level conformance path.
...
set expected_ok {expected_ok}
set got [catch {{db deserialize main $ser}} msg]
puts "case {case_name}"

if {{$expected_ok}} {{
  if {{$got != 0}} {{
    puts "diverge {case_name} tcl_deserialize_mismatch expected=SQLITE_OK observed=$msg"
  }}
}} else {{
  if {{$got == 0}} {{
    puts "diverge {case_name} tcl_deserialize_mismatch expected={expected_rc} observed=SQLITE_OK"
  }}
}}
